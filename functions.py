...
def find_best_segment_60s(
    signal_1, signal_2, fs, duration=60, step=10, max_amplitude=1.5
):
    ...
    for i in range(0, len(signal_1) - window, step_samples):
        ...
        if (
            np.max(np.abs(seg1)) < max_amplitude
            and np.max(np.abs(seg2)) < max_amplitude
        ):
